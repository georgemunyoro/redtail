...
int main() {
  ...
  for (;;) {
    ...
    if (comm == "isready") {
      ...
      if (position == "startpos") {
        ...
        if (moves == "moves") {
          ...
          for (int i = 1; i < uciMoves.size(); ++i) {
            ...
            int currentMoveMove = (from_pos << 20) | (target_pos << 12) |
                                  (MainBoard.squares[from_pos] << 8) |
                                  (MainBoard.squares[target_pos] << 4) | 0;
      ...
      cout << "bestmove " << GetRef((move >> 20) & 0xff)
           << GetRef((move >> 12) & 0xff) << endl;
    } else if (comm == "listmoves") {
      MoveList mvs = MainBoard.LegalMoves();
      for (int i = 0; i < mvs.count; ++i) {
        int move = mvs.moves[i];
        cout << GetRef((move >> 20) & 0xff) << GetRef((move >> 12) & 0xff)
             << endl;
      }
      cout << mvs.count << endl;
    } else if (comm == "quit") {
